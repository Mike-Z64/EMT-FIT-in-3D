...
def FITGridEven(x_len, y_len, z_len, nx, ny, nz):
    ...
    grid["G_hat"]["x"] = grid["G"]["x"][:] + (dx / 2.0) # previously used grid["G"]["x"][:-1] but we want to keep the same number of points
    grid["G_hat"]["y"] = grid["G"]["y"][:] + (dy / 2.0)
    grid["G_hat"]["z"] = grid["G"]["z"][:] + (dz / 2.0)
...
def plot_grids(grid):
    ...
    # Extract coordinates for easier typing
    ...
    ax2.set_title("XY Plane Cross-section (Z=0)")
    ax2.set_xlabel("X")
    ax2.set_ylabel("Y")
    ...
    ax3.set_title("XZ Plane Cross-section (Y=0)")
    ax3.set_xlabel("X")
    ax3.set_ylabel("Z")
...
def main():
    ...
    grid = FITGridEven(x_len=1.0, y_len=1.0, z_len=1.0, nx=3, ny=3, nz=3)
    C_matrix, S_matrix = topological_matrices(grid)
    ...
    verification_matrix = S_matrix @ C_matrix
