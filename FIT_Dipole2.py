...
def get_node_index(i, j, k, nx, ny):
    """
    Calculates the 1D global node index 'n' from 3D coordinates (i, j, k) based on grid size (equation from slide deck)
    """
...
#######################################################
## Define source types (point or dipole)             ##
#######################################################

def create_point_source(nx, ny, nz, i, j, k, direction):
    """
    Creates a point source at a specific 3D coordinate.
...
def create_dipole_source(nx, ny, nz, i, j, k, direction, length=1):
    """
    Creates a dipole source using two adjacent edges with opposite excitation.
    The dipole consists of two edges separated by 'length' cells.
    
    Parameters:
    direction: 'x', 'y', or 'z' - orientation of the dipole
    length: number of cells separating the dipole elements (default=1)
    
    Returns: tuple of (index1, index2) for the two edges
    """
    n1 = get_node_index(i, j, k, nx, ny)
    N_total = nx * ny * nz
    
    # Determine the second node based on direction
    if direction == 'x':
        n2 = get_node_index(min(i + length, nx - 1), j, k, nx, ny)
        offset = 0
    elif direction == 'y':
        n2 = get_node_index(i, min(j + length, ny - 1), k, nx, ny)
        offset = N_total
    elif direction == 'z':
        n2 = get_node_index(i, j, min(k + length, nz - 1), nx, ny)
        offset = 2 * N_total
    else:
        raise ValueError("Direction must be 'x', 'y', or 'z'")
...
def leapfrog_scheme(e_initial, h_initial, b, d, C, C_hat, M_eps, M_mu, delta_t, n_steps, frequency, source_index, pec_indices=None):
    ...
    - For dipole sources, two edges are excited with opposite phases (180° apart).
    """
    ...
    if isinstance(source_index, tuple):
        ...
        print(f"  -> Applying POINT source excitation at index {source_index}")
    ...
    for n in range(n_steps):
        ...
        # Equation: d^(n+1) = d^n + delta_t * (C_hat * h^(n+1/2) - j_source^(n+1/2)) ( Source implemented elsewhere)
        ...
        src_voltage = amplitude * np.sin(omega * curr_time)  # Sinusoidal voltage source, instead of J_source
        
        # Apply source excitation
        if is_dipole:
            # For dipole: opposite phases on two edges (180° phase difference)
            e[n+1, source_idx1] = src_voltage
...
def animate_slice(history_array, grid, plane, slice_index, component='z', is_db=True, save_as_gif=None):
    """
    Creates an interactive plot of a specific field component in a chosen plane with sliders to control frames and colorbar limits.
    ...
    - save_as_gif: filename to save animation as GIF (e.g., 'animation.gif'). If None, only displays.
    """
    nx, ny, nz = grid["nx"], grid["ny"], grid["nz"]
    N_total = nx * ny * nz
    ...
    print("Frames extracted! Creating interactive plot...")

    # 4. Setup the Interactive Figure with Sliders
    fig, (ax, ax_slider_time, ax_slider_vmin, ax_slider_vmax) = plt.subplots(4, 1, figsize=(7, 10), 
                                                                             gridspec_kw={'height_ratios': [8, 1, 1, 1]})
    
    # Choose colormap based on whether we are plotting dB or raw fields
    if is_db:
        cmap = 'jet'
        initial_vmin, initial_vmax = -130, 0
        label = f'Field Intensity (dB)'
    else:
        cmap = 'seismic'
        max_val = np.max(np.abs(frames))
        initial_vmin, initial_vmax = -max_val, max_val
        label = f'Field Amplitude'
        
    im = ax.imshow(frames[0], origin='lower', cmap=cmap, 
                   vmin=initial_vmin, vmax=initial_vmax, extent=extent)
    
    cbar = fig.colorbar(im, ax=ax, label=label, shrink=0.8)
    title = ax.set_title(f'{plane.upper()} Cross-Section of {component}-component\nTime Step = 0')
    ax.set_xlabel(xlabel)
    ax.set_ylabel(ylabel)

    # Create sliders
    slider_time = Slider(ax_slider_time, 'Time Step', 0, total_steps-1, valinit=0, valstep=1)
    slider_vmin = Slider(ax_slider_vmin, 'Vmin', np.min(frames), np.max(frames), valinit=initial_vmin)
    slider_vmax = Slider(ax_slider_vmax, 'Vmax', np.min(frames), np.max(frames), valinit=initial_vmax)
    
    # Update function for all sliders
    def update(val):
        frame = int(slider_time.val)
        vmin = slider_vmin.val
        vmax = slider_vmax.val
        im.set_array(frames[frame])
        im.set_clim(vmin, vmax)
        title.set_text(f'{plane.upper()} Cross-Section of {component}-component\nTime Step = {frame}')
        cbar.update_normal(im)  # Update colorbar
    ...
    if save_as_gif:
        print(f"Saving animation as {save_as_gif}...")
        # For saving, we'll still use the animation approach
        def update_for_anim(frame):
            im.set_array(frames[frame])
            title.set_text(f'{plane.upper()} Cross-Section of {component}-component\nTime Step = {frame}')
            return [im, title]

        ani = animation.FuncAnimation(fig, update_for_anim, frames=total_steps, interval=50, blit=True)
        ani.save(save_as_gif, writer='pillow')
...
def main(source_type='dipole'):
    """
    Main simulation function.
    
    Parameters:
    source_type: 'point' for point source, 'dipole' for dipole source (default='dipole')
    """
    # grid parameters
    ...
    # Calculate frequency to make dipole lambda/2 based on length in nodes
    c = 3e8  # speed of light in m/s
    dipole_length = 18
    spacing = grid['G']['dz']  # assuming z orientation
    L = dipole_length * spacing
    frequency = c / (2 * L) # frequency determined by dipole length
    ...
    """
    # The following is a test for the node to indexs conversion. 
    ...
    if source_type.lower() == 'point':
        ...
        print(f"\n Source Definition ")
        print(f"Point source placed at ({center_x}, {center_y}, {center_z}) pointing in {orientation}.")
        print(f"Edge index: {source_index}")
    elif source_type.lower() == 'dipole':
        # Offset the starting position so the middle of the dipole is at the center
        dipole_offset = dipole_length // 2
        if orientation == 'x':
            dipole_pos = (center_x - dipole_offset, center_y, center_z)
        elif orientation == 'y':
            dipole_pos = (center_x, center_y - dipole_offset, center_z)
        else:  # 'z'
            dipole_pos = (center_x, center_y, center_z - dipole_offset)
        
        source_index = create_dipole_source(nx, ny, nz, dipole_pos[0], dipole_pos[1], dipole_pos[2], orientation, length=dipole_length)
        
        # Calculate physical length of the dipole
        spacing = grid['G']['d' + orientation]
        L_physical = dipole_length * spacing
        wavelength = c / frequency
        print(f"Dipole length: {L_physical:.4f} m")
        print(f"Dipole length: {L_physical / wavelength:.4f} λ")
    ...
    # Plot E_x,y,z in the XZ,XY,YZ plane (slicing through the center X,Y,Z-coordinate)
